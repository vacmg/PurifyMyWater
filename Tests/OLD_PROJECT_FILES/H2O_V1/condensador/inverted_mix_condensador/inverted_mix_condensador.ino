...
const String data = "H20_Software_v_1.0_ST_by_Victor_Arzoz";
...
unsigned const int d = 100; // delay serial (100)
...
unsigned const int vRele = 53; //53
//unsigned const int rRele = 43;
unsigned const int inversor = 51;

unsigned const int ssr = 10; //10
...
unsigned const int positive = 11;
...
void setup()
{
  ...
  digitalWrite(vRele,modoOutput); // no carga
  digitalWrite(inversor,modoOutput); // apagado
  digitalWrite(ssr,modoOutput); // no carga (creo)///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  digitalWrite(ssr,0); // no carga (creo)//////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //pinMode(ssr,OUTPUT); //0-25 // is declared before bootanimation
...
void loop()
{
  ...
  if (work == 0)
  {
    ...
    if (ePulsador == 1 && eBoyaEndAb == 0 && volt > 7)
    {
      eCuboEnd = 0;
      digitalWrite(grifo,0); // abre
      delay(500);
      for (int i = 0; i<s && eBoyaEndAb == 0;i++) 
      {
        vital();
        if (ePulsador == 1)
        {
      ...
      while( ePulsador == 1 && eBoyaEndAb == 0){vital();}
  ...
  else
  {
    ...
    if (eBoyaStartAr == 0 && eBoyaStartAb == 0 && eBoyaUVAr == 1)// filtro // poner voltaje min
    {
      if (volt > 13.5) // si no funciona, quitar
      {
        if (eBoyaUVAr == 1)
        {
          off();
          ...
          atime2 = millis();
          while(eBoyaStartAb == 0 && eBoyaUVAr == 1)
          {
            atime2 = millis();
            vital();
            if (volt < 12)
    ...
    if (ePulsador == 1 && eBoyaEndAb == 0)
    {
      ...
      {
        vital();
        if (eBoyaUVAb == 0 && eBoyaEndAr == 1 && u == 0)
        {
          u = 1;
          search4();
          eUV = 1;
        ...
        if ((eBoyaUVAb == 1 || eBoyaEndAr == 0)&& v == 0)
        {
          v = 1;
          eUV = 0;
          time3 = 0;
      ...
      while( ePulsador == 1 && eBoyaEndAb == 0)
      {
        vital();
        if (eBoyaUVAb == 0 && eBoyaEndAr == 1 && u == 0)
        {
...
void refill(bool r)
{
  if (r == 0)
  {
    while(eBoyaStartAr == 1)
    {
      digitalWrite(bombaPozo,0); // enciende
      vital();
      while (eBoyaStartAr == 1 && eBoyaSH == 0)
      {
        vital();
      }
      digitalWrite(bombaPozo,1); // apaga
