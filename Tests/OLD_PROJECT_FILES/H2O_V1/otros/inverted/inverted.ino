...
unsigned long interval_1 = 300; //(seg)
unsigned long interval_2 = 60; //(seg)
unsigned long interval_3 = 300; //(seg)
...
void loop()
{
  ...
  else
  {
    ...
    if ((eBoyaStartAr == 0 || eBoyaUVAb == 1) && eBoyaStartAb == 0)// filtro // poner voltaje min
    {
      if (volt > 12.5) // si no funciona, quitar
      {
        if (eBoyaUVAr == 1)
        {
          ...
          atime2 = millis();
          bool x = 0;
          while(eBoyaStartAb == 0 && eBoyaUVAr == 1 && x == 0)
          {
            atime2 = millis();
            vital();
            if (volt < 11.5)
    ...
    if ((eBoyaUVAb == 0 && eBoyaEndAr == 1) && y == 0)
    {
      y = 1;
      search4();
      eUV = 1;
    ...
    if ((eBoyaUVAb == 1 || eBoyaEndAr == 0) && y == 1)
    {
      y = 0;
      eUV = 0;
      time3 = 0;
