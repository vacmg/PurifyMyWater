    ...
    void drawTemperature() {
        ...
        switch (page) {
            case 1:
                ...
                draw6ButtonsLayout(F("Temp. Refresh Rate "), F("System Stop Temp."), F("PSU Fan Start Temp."),
                                   String(((double) TEMPCHECKTIME) / 1000) + "s",
                                   String((double) STOPWORKINGTEMP) + "C", String((double) STARTPSUTEMP) + "C",
                                   true, true, true, fontSizes);
                break;
            case 2:
                setFontSizeArray(fontSizes, 1, 1, 1, 2, 2, 2);
                draw6ButtonsLayout(F("PSU Fan Stop Temp."), F("Case Fan Start Temp."), F("Case Fan Stop Temp."),
                                   String((double) STOPPSUTEMP) + "C", String((double) STARTCASETEMP) + "C",
        ...
        if (btn4.isPressed()) {
            switch (page) {
                case 1: // Temp. Refresh Rate
                    ...
                    {
                        if (tempVal > 0) { debug(
                                    String(F("TEMPCHECKTIME UPDATED: ")) + TEMPCHECKTIME + String(F(" --> ")) +
                                    tempVal);
                ...
                case 2: //PSU Fan Stop Temp.
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (tempVal > 0 && tempVal < STARTPSUTEMP) { debug(
                                    String(F("STOPPSUTEMP UPDATED: ")) + STOPPSUTEMP + String(F(" --> ")) +
                                    tempVal);
            ...
            switch (page) {
                case 1: // System Stop Temp.
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (tempVal > 0 && tempVal > STARTPSUTEMP && tempVal > STARTCASETEMP) { debug(
                                    String(F("STOPWORKINGTEMP UPDATED: ")) + STOPWORKINGTEMP + String(F(" --> ")) +
                                    tempVal);
                ...
                case 2: // Case Fan Start Temp.
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (tempVal > 0 && tempVal > STOPCASETEMP && tempVal < STOPWORKINGTEMP) { debug(
                                    String(F("STARTCASETEMP UPDATED: ")) + STARTCASETEMP + String(F(" --> ")) +
                                    tempVal);
            ...
            switch (page) {
                case 1: // PSU Fan Start Temp.
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (tempVal > 0 && tempVal > STOPPSUTEMP && tempVal < STOPWORKINGTEMP) { debug(
                                    String(F("STARTPSUTEMP UPDATED: ")) + STARTPSUTEMP + String(F(" --> ")) +
                                    tempVal);
                ...
                case 2: //Case Fan Stop Temp.
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (tempVal > 0 && tempVal < STARTCASETEMP) { debug(
                                    String(F("STOPPSUTEMP UPDATED: ")) + STOPCASETEMP + String(F(" --> ")) +
                                    tempVal);
