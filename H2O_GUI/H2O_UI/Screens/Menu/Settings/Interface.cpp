    ...
    void drawInterface() {
        ...
        switch (page) {
            case 1:
                ...
                draw6ButtonsLayout(F("Language"), F("Screen Rotation"), F("Screen Calibration"), F("English"),
                                   F("Inverted Landscape"), F("Calibrate"), true, true, true, fontSizes);
                break;
            case 2:
                setFontSizeArray(fontSizes, 2, 2, 2, 2, 2, 2);
                draw6ButtonsLayout(F("Refresh Period"), F("Reset"), "",
                                   (String) (((double) DATAREFRESHPERIOD) / 1000) + "s", "Perform Reset", "", true,
                                   true, false, fontSizes);
                break;
        }
        ...
        if (btn4.isPressed()) {
            switch (page) {
                case 2: // Refresh Period
                    ...
                    {
                        if (tempVal > 0) { debug(
                                    String(F("DATAREFRESHPERIOD UPDATED: ")) + DATAREFRESHPERIOD + String(F(" --> ")) +
                                    tempVal);
