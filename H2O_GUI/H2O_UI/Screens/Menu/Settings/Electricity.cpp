        ...
        if (btn4.isPressed()) {
            switch (page) {
                case 1: // Start charging voltage
                    ...
                    {
                        if (STOPWORKINGVOLTAGE < tempVal && tempVal < STOPCHARGINGVOLTAGE) { debug(
                                    String(F("STARTCHARGINGVOLTAGE UPDATED: ")) + STARTCHARGINGVOLTAGE +
                                    String(F(" --> ")) + tempVal);
                ...
                case 2: //page 2
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (STARTCHARGINGVOLTAGE < tempVal && tempVal <
                                                              STOPCHARGINGVOLTAGE)// STARTCHARGINGVOLTAGE < STARTWORKINGVOLTAGE < STOPCHARGINGVOLTAGE
                        { debug(String(F("STARTCHARGINGVOLTAGE UPDATED: ")) + STARTWORKINGVOLTAGE + String(F(" --> ")) +
                                tempVal);
            ...
            switch (page) {
                case 1:
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (STARTCHARGINGVOLTAGE + 1 < tempVal && tempVal <
                                                                  MAXCAPACITORSALLOWEDVOLTAGE)// STARTCHARGINGVOLTAGE+1 < STOPCHARGINGVOLTAGE < MAXCAPACITORSALLOWEDVOLTAGE
                        { debug(String(F("STOPCHARGINGVOLTAGE UPDATED: ")) + STOPCHARGINGVOLTAGE + String(F(" --> ")) +
                                tempVal);
                ...
                case 2:
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        if (MINSYSTEMALLOWEDVOLTAGE < tempVal && tempVal < STARTCHARGINGVOLTAGE -
                                                                           1)// MINSYSTEMALLOWEDVOLTAGE < STOPWORKINGVOLTAGE < STARTCHARGINGVOLTAGE-1
    ...
    void drawElectricity() // TODO get settings real value
    {
        ...
        switch (page) {
            case 1:
                ...
                draw6ButtonsLayout(F("Start Charging Voltage"), F("Stop Charging Voltage"), F("UV light est. Current"),
                                   String(STARTCHARGINGVOLTAGE) + F("V"), String(STOPCHARGINGVOLTAGE) + F("V"),
                                   String(ESTIMATEDUVAMPERAGE) + F("A"), true, true, true, fontSizes);
                break;
            case 2:
                setFontSizeArray(fontSizes, 1, 1, 1, 2, 2, 2);
                draw6ButtonsLayout(F("Start Working Voltage"), F("Stop Working Voltage"), F("AC Inverter Frequency"),
                                   String(STARTWORKINGVOLTAGE) + F("V"), String(STOPWORKINGVOLTAGE) + F("V"),
                                   String(ACFREQUENCY) + F("Hz"), true, true, true, fontSizes);
                break;
            case 3:
                setFontSizeArray(fontSizes, 1, 1, 1, 2, 2, 2);
                draw6ButtonsLayout(F("AC Ammeter Sensitivity"), F("AC Ammeter Zero"), F("DC Ammeter Sensitivity"),
                                   String(ACAMPSENSITIVITY, 4), String(ACAMPZERO, 4), String(DCAMPSENSITIVITY, 4), true,
                                   true, true, fontSizes);
                break;
            case 4:
                setFontSizeArray(fontSizes, 1, 1, 1, 2, 1, 1);
                draw6ButtonsLayout(F("DC Ammeter Zero"), "", "", String(DCAMPZERO, 4), "", "", true, false, false,
                                   fontSizes);
                break;
        }
    }
