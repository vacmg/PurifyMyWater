            ...
            switch (page) {
                case 1:
                    ...
                    if (!isnan(tempVal)) // if getNumInput was not cancelled
                    {
                        ...
                        { debug(String(F("WELLPUMPTIMEOUT UPDATED: ")) + WELLPUMPTIMEOUT + String(F(" --> ")) +
                                tempVal);
    ...
    void drawWater() {
        ...
        switch (page) {
            case 1:
                ...
                draw6ButtonsLayout(F("Well Pump max time ON"), F("UV Pump max time ON"), F("End Pump max time ON"),
                                   String(((double) WELLPUMPTIMEOUT) / 1000) + F("s"),
                                   String(((double) UVPUMPTIMEOUT) / 1000) + F("s"),
                                   String(((double) ENDPUMPTIMEOUT) / 1000) + F("s"), true, true, true, fontSizes);
                break;
            case 2:
                setFontSizeArray(fontSizes, 1, 1, 1, 2, 2, 2);
                draw6ButtonsLayout(F("Filter max time ON"), F("UV Pump flow"), "",
                                   String(((double) FILTERTIMEOUT) / 1000) + F("s"), String(UVPUMPFLOW) + F("L/H"), "",
