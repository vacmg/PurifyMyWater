...
// Draw voltage & amount of purified water
    ...
    void drawStatusColors(bool wellPump, bool endPump, bool UVRelay, bool filterRelay, char well, char surfaceTank,
                          char filteredTank, char purifiedTank,
                          bool endTank) // false --> OFF, true -->ON, <0 --> LOW, = 0 --> Half, >0 --> FULL
    {
    ...
    void clickStatus() {
        if (btn1.isPressed()) { debug(F("Button MENU pressed"));
            ...
            if (mainSwitchSt == ON) { debug(F("Button OFF pressed"));
                mainSwitchSt = OFF; // TODO send off command
                drawStatusForeground("15.4V", "320L");
            } else if (mainSwitchSt == OFF) { debug(F("Button ON pressed"));
                mainSwitchSt = ON; // TODO send on command
                drawStatusForeground("15.4V", "320L");
            } else if (mainSwitchSt < 0) { debug(F("Button FAILURE pressed")); // TODO Draw ERROR message
            }
            delay(500);
        }
    }
