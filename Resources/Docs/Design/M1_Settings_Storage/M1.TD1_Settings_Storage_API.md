...
- **public SettingError_t printSettingsTree(FILE\* fp, const std::string& key)**
  // This prints a formatted string to the specified FILE using fmt library with the subtree that hangs from key (in a recursive way):
  ```
  KEY_STR
  |-SUBKEY1_STR --- VALUE1
  |-SUBKEY2_STR --- VALUE2
  ...
  - If key is not a SETTINGS_MAP entry, it prints only the entry pointed by key (KEY_STR --- VALUE) and return NO_ERROR.
  - If the key points to a SETTINGS_MAP entry, it prints the whole subtree and return NO_ERROR.
  - If the key is '/', nullptr or '': it prints the complete settings tree and return NO_ERROR.
  - If the key does not exist: it won't print anything and return KEY_NOT_FOUND_ERROR.
